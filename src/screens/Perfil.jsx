...
export function PerfilScreen({ perfilData, onSalvar, onSairDoGrupo, onSairDaConta, onFechar }) {
  ...
  const salvar = async () => {
    ...
    setErro('')
    try {
      await onSalvar({ nome: nome.trim(), telefone: telefone.trim() || null, chavePix: chavePix.trim() || null, posicao: posicao || null })
      onFechar()
    } catch (err) {
      setErro(err.message)
    } finally {
  ...
  return (
    ...
    >
      <div style={{ flex: 'none', padding: '62px 20px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ font: "800 22px/1 'Barlow Condensed', sans-serif" }}>Meu perfil</div>
